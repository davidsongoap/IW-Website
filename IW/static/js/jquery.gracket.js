...
(function ($) {
    $.fn.gracket = function (method) {
        ...
        var container = this,
            data =
                typeof container.data("gracket") === "undefined"
                    ? []
                    : typeof container.data("gracket") === "string"
                    ? JSON.parse(container.data("gracket"))
        ...
        var methods = {
            init: function (options) {
                ...
                if (this.gracket.settings.src.length) data = this.gracket.settings.src;

                // always prepend unique id to canvas id, as we dont want dupes
                this.gracket.settings.canvasId =
                    this.gracket.settings.canvasId + "_" + new Date().getTime();
                ...
                container.addClass(this.gracket.settings.gracketClass).prepend(_canvas);
                ...
                for (var r = 0; r < round_count; r++) {
                    ...
                    for (var g = 0; g < game_count; g++) {
                        var game_html = helpers.build.game(this.gracket.settings),
                            outer_height = container
                                .find("." + this.gracket.settings.gameClass)
                                .outerHeight(true),
                            spacer = helpers.build.spacer(
                                this.gracket.settings,
                                outer_height,
                                r,
                                r !== 0 && g === 0
                            );
                        ...
                        for (var t = 0; t < team_count; t++) {
                            ...
                            if (team_count === 1) {
                                ...
                                // align winner
                                helpers.align.winner(
                                    game_html,
                                    this.gracket.settings,
                                    game_html.parent().prev().children().eq(0).height()
                                );
                                ...
                                helpers.listeners(
                                    this.gracket.settings,
                                    data,
                                    game_html.parent().prev().children().eq(1)
                                );
                            }
                        }
                    }
        ...
        var helpers = {
            build: {
                ...
                spacer: function (node, yOffset, r, isFirst) {
                    return (spacer = $("<div />", {
                        ...
                        height: isFirst
                            ? (Math.pow(2, r) - 1) * (yOffset / 2)
                            : (Math.pow(2, r) - 1) * yOffset,
                    }));
                },
                    ...
                    for (i = 0; i < len; i++) {
                        left =
                            i === 0
                                ...
                                : off.padding + widthPadding + off.right * i;
                        $("<h5 />", {
                            // html: off.labels.length ? off.labels[i] : "Round" + (i + 1),
                            html: off.labels.length ? off.labels[i] : (i < len - 1) ? "Round " + (i + 1) : "Winner!",
                            class: off["class"],
                        })
                            .css({
                                position: "absolute",
                ...
                canvas: {
                    resize: function (node) {
                        ...
                        console.log("CANVAS WIDTH " + container.innerWidth());
                        console.log("CANVAS HEIGHT " + container.innerHeight());
                        $(canvas).css({
                    ...
                    draw: function (node, data, game_html) {
                        ...
                        var _itemWidth = max_round_width[0],
                            ...
                            _marginRight =
                                parseInt(container.find("> div").css("marginRight")) || 0,
                            _cornerRadius = node.cornerRadius,
                            _lineGap = node.canvasLineGap,
                            _playerGap =
                                game_html.height() - 2 * game_html.find("> div").eq(1).height();
                        (_playerHt = game_html.find("> div").eq(1).height()),
                            (_totalItemWidth = 0);

                        if (typeof console !== "undefined")
                            console.info(
                                "Padding Left: " + _paddingLeft + "px",
                                "Player/Name Width: " + _itemWidth + "px",
                                "Container padding left: " + _startingLeftPos + "px"
                            );
                        ...
                        var p = Math.pow(2, data.length - 2),
                            i = 0,
                            j,
                            r = 0.5,
                        ...
                        while (p >= 1) {
                            for (j = 0; j < p; j++) {
                                ...
                                var xInit = ifOneGame
                                    ? _itemWidth + _paddingLeft
                                    : _startingLeftPos + _totalItemWidth + i * _marginRight,
                                    xDisp = r * _marginRight,
                                    yInit =
                                        ((Math.pow(2, i - 1) - 0.5) * (i && 1) +
                                            j * Math.pow(2, i)) *
                                        _itemHeight +
                                        _paddingTop +
                                        (ifOneGame
                                            ? _ref.find("> div").eq(1).height()
                                            : _playerHt) +
                                        _playerGap / 2;
                                if (p > 1) {
                                ...
                                if (p > 1 && j % 2 == 0) {
                                    ...
                                    var yBottom =
                                        yInit + Math.pow(2, i) * _itemHeight - _cornerRadius;
            ...
            align: {
                winner: function (game_html, node, yOffset) {
                    var ifOneGame =
                        game_html.parent().siblings().not("canvas").length === 1;
                    var offset = ifOneGame
                        ? yOffset - (game_html.height() + game_html.height() / 2)
                        : yOffset + game_html.height() / 2;
